...
void analyzeAndStore(double *real, double *imag, ArduinoFFT<double> &fft, const char *label, float vv);
...
void loop()
{
  ...
  analyzeAndStore(vRealX, vImagX, FFT_X, "X", vx);
  analyzeAndStore(vRealY, vImagY, FFT_Y, "Y", vy);
  analyzeAndStore(vRealZ, vImagZ, FFT_Z, "Z", vz);
